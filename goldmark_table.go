...
/**
 * 单独实现了 goldmark 的 table extension，以支持多行表头，及 colspan 和 rowspan。
 * table 前最好以空行分隔，比较保险一些 :P。
 * 行列 span 类似于下面：
 * |2] 市    |2} 男生    |2} 女生   |
 * 竖线后紧跟数字及']'，表示 rowspan，若是‘}’表示 colspan。
 * 同时使用时，需要将 colspan 放在前面，如跨 3 列 2 行，则写：
 * |3}2]
 * 注意：因为存在行 span，所以，不能判断一行的 td 个数是否合理，需要 md 文档自己保证
 */
...
func (b *tableParagraphTransformer) Transform(node *gast.Paragraph, reader text.Reader, pc parser.Context) {
  ...
  for i := 1; i < lines.Len(); i++ {
    ...
    headLineCount := 1
    for j := i - 2; j >= 0; j-- {
      header = b.parseRow(lines.At(j), alignments, true, reader, pc)
      if header == nil {
        break
      }
      table.InsertBefore(table, table.FirstChild(), ast.NewTableHeader(header))
      headLineCount++
    }
    ...
    node.Lines().SetSliced(0, i-headLineCount)
    node.Parent().InsertAfter(node.Parent(), node, table)
    if node.Lines().Len() == 0 {
      node.Parent().RemoveChild(node.Parent(), node)
    } else {
      last := node.Lines().At(i - headLineCount - 1)
      last.Stop = last.Stop - 1 // trim last newline(\n)
      node.Lines().Set(i-headLineCount-1, last)
    }
  }
}

var colSpanReg = regexp.MustCompile(`^(\d+)}`)
var rowSpanReg = regexp.MustCompile(`^(\d+)]`)
...
func (b *tableParagraphTransformer) parseRow(segment text.Segment,
  ...
  for ; pos < limit; i++ {
    ...
    line2 := string(line[pos:])
    if colSpanReg.MatchString(line2) {
      match := colSpanReg.FindStringSubmatch(line2)
      node.SetAttributeString("colspan", []byte(match[1]))
      pos += len(match[0])
      line2 = string(line[pos:])
    }
    if rowSpanReg.MatchString(line2) {
      match := rowSpanReg.FindStringSubmatch(line2)
      node.SetAttributeString("rowspan", []byte(match[1]))
      pos += len(match[0])
    }
...
func (r *TableHTMLRenderer) renderTableHeader(
  ...
  if entering {
    ...
    if n.NextSibling() == nil {
      _, _ = w.WriteString("</thead>\n")
    } else if n.NextSibling().Kind() != ast.KindTableHeader {
      _, _ = w.WriteString("</thead>\n")
